...
const AdminNav: FC<Props> = ({ navItems }): JSX.Element => {
    ...
    return <nav ref={navRef} className='h-screen w-60 shadow-sm bg-secondary-light dark:bg-secondary-dark flex flex-col justify-between transition-width overflow-hidden sticky top-0'>
        <div>
            <Link href='/admin' className='flex items-center space-x-2 p-3 mb-10'>
                <Logo className='fill-highlight-light dark:fill-highlight-dark w-5 h-5' />
                {visible && <span className='text-highlight-light dark:text-highlight-dark text-xl font-semibold leading-none'>Admin</span>}
            </Link>
            <div className='space-y-6'>
                {
                    navItems.map((item) => {
                        return (
                            <Link key={item.href} href={item.href} className='flex items-center text-highlight-light dark:text-highlight-dark text-xl p-3 hover:scale-[0.98] transition'>
                                <item.icon size={24} />
